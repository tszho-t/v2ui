...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget -q -progress -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/XrayR-project/XrayR/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        wget -q -progress -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip ${url}
        if [[ $? -ne 0 ]]; then
    ...
    wget -q -progress  -N --no-check-certificate -O /etc/systemd/system/XrayR.service ${file}
