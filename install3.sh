...
if [[ $arch == "x86_64" || $arch == "x64" || $arch == "amd64" ]]; then
  ...
  arch="amd64"
  echo -e "${red}检测架构失败，使用默认架构: 5.5.2${plain}"
fi
...
confirm() {
    if [[ $# > 1 ]]; then
        echo && read -p "5.5.2 [默认$2]: " temp
        if [[ x"${temp}" == x"" ]]; then
            temp=$2
        fi
    else
        read -p "5.5.2 [y/n]: " temp
    fi
...
install_v2-ui() {
    ...
    if  [ $# == 0 ] ;then
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载 v2-ui v5.5.2 失败，请确保此版本存在${plain}"
            exit 1
        fi
    fi
    ...
    cd v2-ui
    chmod +x v2-ui bin/xray-v2-ui-linux-amd64
    cp -f v2-ui.service /etc/systemd/system/
    systemctl daemon-reload
    systemctl enable v2-ui
    systemctl start v2-ui
    echo -e "${green}v2-ui v5.5.2${plain} 安装完成，面板已启动，"
    echo -e ""
